...
**目的**: v0.2.x から v0.3.x への移行ガイドを作成する

**詳細**:
- 破壊的変更のリスト
- 自動シャドウ化への移行手順
- shadow.config.json の廃止に関する説明
- コード例の before/after
...
2. ✅ タスク2: マイグレーションガイドの作成
3. タスク3: コントリビューションガイドの作成
4. タスク4: セキュリティポリシーの作成

### 中優先度（次のマイナーバージョン）

5. タスク5: 自動シャドウ化のテスト追加
6. タスク6: エラーハンドリングのテスト追加
7. タスク11: 構造化ロギングの追加

### 低優先度（将来のバージョン）

8. タスク7: バルク操作のパフォーマンス最適化
9. タスク8: シャドウレコード生成の最適化
10. タスク9: クエリビルダーの改善
11. タスク10: トランザクションサポートの追加
12. タスク12: メトリクスの追加
13. タスク13: CLIツールの拡張
14. タスク14: VSCode拡張の作成
...
1. 高優先度タスク（タスク3-4）を実施
2. テストカバレッジを80%以上に向上（タスク5-6）
3. v0.4.0リリースに向けて機能拡張を検討（タスク7-14）
