...
- ✅ 要件24: アーキテクチャのリファクタリング（v0.5.0で実装）
- ✅ 要件25: updateOne/updateManyのupsertオプション対応（v0.9.0で実装）
...
- [x] 7.2 プロジェクト構造の説明
- [x] 7.3 開発ワークフローの定義
- [x] 7.4 コーディング規約（TypeScript、命名規則、コメント）
- [x] 7.5 テストの書き方とベストプラクティス
- [x] 7.6 プルリクエストのプロセスとチェックリスト
- [x] 7.7 Conventional Commitsに基づくコミットメッセージ規約
- [x] 7.8 リリースプロセスとバージョニング
- [x] 7.9 コミュニティガイドラインと行動規範

**完了日**: 2024-12-23
**成果**: 
- 包括的なコントリビューションガイド（`CONTRIBUTING.md`）を作成
- 開発環境のセットアップから本番リリースまでの完全なワークフロー
- TypeScript、命名規則、JSDocコメントのコーディング規約
- AAA パターンに基づくテストガイドライン
- プルリクエストのプロセスとレビューチェックリスト
- Conventional Commitsに基づくコミットメッセージ規約
- Semantic Versioningに基づくリリースプロセス
- コミュニティガイドラインと行動規範
...
**サブタスク**:
- [x] 8.1 サポートバージョンの明確化
- [x] 8.2 脆弱性報告の手順（GitHub Security Advisories）
- [x] 8.3 責任ある開示プロセスとタイムライン
- [x] 8.4 開発者向けセキュリティベストプラクティス
- [x] 8.5 利用者向けセキュリティガイドライン
- [x] 8.6 既知のセキュリティ考慮事項
- [x] 8.7 セキュリティアップデートの通知と適用方法
- [x] 8.8 監査とコンプライアンス

**完了日**: 2024-12-23
**成果**: 
- セキュリティポリシー（`SECURITY.md`）を作成
- サポートバージョンとライフサイクルの明確化
- GitHub Security Advisoriesを使用した脆弱性報告手順
- 責任ある開示プロセスと協調的開示タイムライン
- 認証、入力検証、ログ記録のセキュリティベストプラクティス
- HTTPS、トークン管理、タイムアウト設定のガイドライン
- OWASP、NIST、CWEに基づくセキュリティ標準への準拠
...
### タスク9.1: parameterConverterのコード品質改善（v0.9.2）

**目的**: parameterConverterモジュールのコード品質と保守性を向上させる

**詳細**:
- MongoDB風APIパラメータ変換処理のリファクタリング
- 三項演算子のフォーマット統一（複数行→単一行）
- upsertオプションの伝播処理の追加
- コードの可読性向上
...
**成果物**:
- `src/server/operations/parameterConverter.ts` - コード品質改善

**サブタスク**:
- [x] 9.1.1 MongoUpdateParamsインターフェースにoptionsフィールド追加
- [x] 9.1.2 convertUpdateOneParamsでoptionsを返り値に含める
- [x] 9.1.3 convertUpdateManyParamsでoptionsを返り値に含める
- [x] 9.1.4 三項演算子のフォーマット統一（可読性向上）
- [x] 9.1.5 テストの実行と確認（314件のテストが通過）

**完了日**: 2024-12-31
**成果**: 
- parameterConverterのコード品質を改善
- upsertオプションがMongoDB風APIから内部操作に正しく伝播されることを確認
- 三項演算子を単一行にまとめて可読性を向上
- 一貫したコーディングスタイルを維持
- すべてのテスト（314件）が正常に通過することを確認
- v0.9.2としてnpm公開
...
### タスク19: $プレフィックス付きオペレータへの移行（v1.0.0）

**目的**: MongoDBとの完全な互換性を実現するため、すべての操作オペレータに `$` プレフィックスを追加する

**詳細**:
- 型定義の更新（FilterOperators、UpdateOperators、Filter）
- サーバー側実装の更新（クエリ変換、更新演算子処理）
- クライアント側実装の更新（Collection、FindCursor）
- テストの更新（すべてのテストケース）
- ドキュメントの更新（README、API.md、使用例）
- マイグレーションガイドの作成
- バージョンアップ（v1.0.0）

**参照**: 要件26（MongoDB互換性向上 - $プレフィックス付きオペレータ）

**成果物**:
- `src/shared/types/index.ts` - 型定義の更新
- `src/server/query/converter.ts` - クエリ変換の更新
- `src/server/operations/*.ts` - 更新演算子処理の更新
- `src/client/Collection.ts` - クライアント実装の更新
- `__tests__/**/*.test.ts` - すべてのテストの更新
- `docs/MIGRATION_v1.md` - マイグレーションガイド
- `CHANGELOG.md` - 破壊的変更の明記
- `package.json` - バージョン1.0.0

**サブタスク**:
- [x] 19.1 型定義の更新（FilterOperators、UpdateOperators、Filter）
...
- [x] 19.2 サーバー側クエリ変換の更新
  - `src/server/query/converter.ts` の更新
  - フィルタ演算子の `$` プレフィックス対応
  - 論理演算子の `$` プレフィックス対応
  - _要件: 26.1, 26.3_
  - _完了: 2024-12-31_

- [x] 19.3 サーバー側パラメータ変換の更新
  - `src/server/operations/parameterConverter.ts` の更新
  - `$set`, `$in` の処理
  - _要件: 26.2_
  - _完了: 2024-12-31_

- [x] 19.4 react-admin統合の更新
  - `src/integrations/react-admin/dataProvider.ts` の更新
  - `$set`, `$in` の使用
  - _要件: 26.5_
  - _完了: 2024-12-31_

- [x] 19.5 テストの更新
  - `__tests__/collection.test.ts` の更新
  - `__tests__/handler.test.ts` の更新
  - `__tests__/converter.test.ts` の更新
  - `__tests__/integration-webapp.test.ts` の更新
  - `__tests__/integration-lambda.test.ts` の更新
  - `__tests__/find-utils.test.ts` の更新
  - すべてのテストケースで `$` プレフィックスを使用
  - _要件: 26.4, 26.5_
  - _完了: 2024-12-31_
  - _結果: 全314テストが通過_

- [x] 19.6 ドキュメントの更新
  - `README.md` の使用例を更新（確認済み：変更不要）
  - `docs/API.md` のオペレータ一覧を更新
  - `docs/ARCHITECTURE.md` の型定義を更新（確認済み：変更不要）
  - _要件: 26.6_
  - _完了: 2024-12-31_

- [x] 19.7 マイグレーションガイドの作成
  - `docs/MIGRATION_v1.md` の作成
  - 自動マイグレーションスクリプトの提供
  - 手動マイグレーション手順の説明
  - _要件: 26.9_
  - _完了: 2024-12-31_
...
**完了日**: 2025-01-01
**成果**:
- package.jsonのバージョンを1.0.0に更新
- 全314テストが通過することを確認
- ビルドが成功することを確認
- CHANGELOG.mdに破壊的変更を明記
- マイグレーションガイド（docs/MIGRATION_v1.md）を作成
- APIドキュメント（docs/API.md）を更新
- npm公開準備完了（[publish]タグでコミット後に自動公開）

**注意事項**:
- これは**破壊的変更（Breaking Change）**です
- v0.xからv1.0.0へのアップグレードには既存コードの更新が必要です
- TypeScriptコンパイラが自動的にエラーを検出するため、見落としは少ないです
- 大規模プロジェクトでは、機能ごとに段階的に移行することを推奨します
- マイグレーション後は必ず全テストを実行して動作を確認してください
...
### タスク20: $setOnInsert オペレータの実装（v1.1.0）

**目的**: MongoDB互換の `$setOnInsert` オペレータを実装し、upsert時にinsert専用のフィールドとupdate専用のフィールドを明確に分けて指定できるようにする

**詳細**:
- 型定義の更新（UpdateOperators に `$setOnInsert` を追加）
- サーバー側実装（handleUpsertCreate、handleUpsertUpdate の更新）
- クライアント側実装（型定義のみ、処理はサーバー側に委譲）
- テストの追加（単体テスト、統合テスト）
- ドキュメント更新（API.md、使用例）

**参照**: 要件27（$setOnInsert オペレータのサポート）

**成果物**:
- `src/shared/types/index.ts` - UpdateOperators型の更新
- `src/server/operations/updateOne.ts` - upsert処理の更新
- `__tests__/operations/updateOne-setOnInsert.test.ts` - テストの追加
- `docs/API.md` - $setOnInsertの説明追加
- `CHANGELOG.md` - 新機能の追加

**サブタスク**:
- [x] 20.1 型定義の更新
  - `UpdateOperators` に `$setOnInsert?: Partial<T>` を追加
  - _要件: 27.7, 27.8_
  - _完了日: 2026-01-05_

- [x] 20.2 サーバー側実装（handleUpsertCreate）
  - `$set` と `$setOnInsert` をマージ（`$set` が優先）
  - insert時に両方のフィールドを適用
  - _要件: 27.1, 27.3, 27.6_
  - _完了日: 2026-01-05_

- [x] 20.3 サーバー側実装（handleUpsertUpdate）
  - `$setOnInsert` を無視
  - `$set` のみを適用
  - _要件: 27.2, 27.4_
  - _完了日: 2026-01-05_
...
- [x] 20.6 ドキュメント更新
  - `docs/API.md` に `$setOnInsert` の説明を追加
  - 使用例とコードスニペットを追加
  - MongoDB互換性の説明を追加
  - _要件: 27.10_
  - _完了日: 2026-01-05_
...
**完了日**: 2026-01-05
**成果**:
- UpdateOperators型に`$setOnInsert`フィールドを追加
- handleUpsertCreateで`$set`と`$setOnInsert`をマージ（`$set`が優先）
- handleUpsertUpdateで`$setOnInsert`を無視
- 包括的なテストカバレッジ（単体テスト・統合テスト）
- API.mdに詳細な説明、使用例、MongoDB互換性の説明を追加
- CHANGELOG.mdにv1.1.0エントリを追加
- 全テストが正常に通過することを確認
...
1. ✅ タスク1-5: アーキテクチャリファクタリング（完了）
2. ✅ タスク6-8: ドキュメント改善（完了）
3. ✅ タスク9: updateOne/updateManyのupsertオプション実装（完了）
4. ✅ タスク19: $プレフィックス付きオペレータへの移行（v1.0.0）（完了）

### 高優先度（次のマイナーバージョン）

5. **タスク20: $setOnInsert オペレータの実装（v1.1.0）** ← 新規追加

### 中優先度（次のマイナーバージョン）

6. タスク10: エラーハンドリングのテスト追加
7. タスク11: 自動シャドウ化のテスト追加
8. タスク15: 構造化ロギングの追加

### 低優先度（将来のバージョン）

9. タスク12-18: パフォーマンス最適化、機能拡張、開発者体験の向上
