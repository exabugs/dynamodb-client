 ...
 *
 * 使用方法:
 *   pnpm --filter @ainews/core tsx scripts/repair-shadows.ts --resource articles --dry-run
 *   pnpm --filter @ainews/core tsx scripts/repair-shadows.ts --resource articles --id C8CD6F0D81B2B030696E300752 --repair
 *   pnpm --filter @ainews/core tsx scripts/repair-shadows.ts --resource articles --repair
 */
...
async function main() {
  ...
  if (resourceIndex === -1 || !args[resourceIndex + 1]) {
    console.error(
      'Usage: pnpm --filter @ainews/core tsx scripts/repair-shadows.ts --resource <resource> [--id <id>] [--dry-run|--repair]'
    );
    process.exit(1);
  }
