...
describe('dynamodb_find integration', () => {
  ...
  describe('基本的な検索', () => {
    it('collectionのみ指定した場合、全レコードを取得できる', async () => {
      ...
      mockSend.mockResolvedValue({
        Items: [{ PK: 'venues#123', SK: 'venues#123', id: '123', name: 'Test Venue' }],
  ...
  describe('ページネーション機能', () => {
    ...
    it('nextTokenを指定した場合、次ページを取得できる', async () => {
      mockSend.mockResolvedValue({
        Items: [{ PK: 'venues#789', SK: 'venues#789', id: '789', name: 'Venue 3' }],
        ConsumedCapacity: { TableName: 'test-table', CapacityUnits: 1 },
      });
  ...
  describe('複合条件', () => {
    it('filter、sort、paginationを同時に指定できる', async () => {
      mockSend.mockResolvedValue({
        Items: [{ PK: 'venues#123', SK: 'name#A', id: '123', name: 'A Venue', status: 'active' }],
